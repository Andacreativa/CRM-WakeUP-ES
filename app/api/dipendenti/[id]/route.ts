  ...
  try {
    ...
    const dipendente = await prisma.dipendente.update({
      ...
      data: {
        ...
        ...(body.dataNascita !== undefined && { dataNascita: body.dataNascita ? new Date(body.dataNascita) : null }),
        ...
        ...(body.nettoBustaPaga !== undefined && { nettoBustaPaga: parseFloat(body.nettoBustaPaga) || 0 }),
        ...(body.irpf !== undefined && { irpf: parseFloat(body.irpf) || 0 }),
        ...(body.seguridadSocial !== undefined && { seguridadSocial: parseFloat(body.seguridadSocial) || 0 }),
      },
    })
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const spesaIds = pagamenti.map(p => p.spesaId).filter((x): x is number => x !== null)
    if (spesaIds.length) {
